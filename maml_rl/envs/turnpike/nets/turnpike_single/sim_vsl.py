...
import numpy as np
import sumo as sumo
from net.turnpike.VSLController1 import VSLController
from net.turnpike.metrics import getAverageDetectorFlow, getAverageTTC

class Simulation(sumo.Sumo):
    
    def initialize_vsl_controller(self, vslfile, bufferfile):
        return VSLController(vslfile, bufferfile)
    ...
    def __init__(self):
        ...
        self.TTCs = []
        self.avgFlows = []
        self.vsl_controller = self.initialize_vsl_controller('/net/turnpike/turnpike_vision_zones',
                                                             '/net/turnpike/turnpike_buffer_zones'
                                                             )
    ...
    def Run(self, capture=None, savetime=200):    
        for _step in tqdm.trange(self.n_steps, desc=self.time):
            ...
            if self.time == 100:
                self.vsl_controller._update_speed_limit(np.full(len(self.vsl_controller.controlzones), 20))
                print ('Speed limit actions applied!')
            elif self.time == 200:
                self.vsl_controller._update_speed_limit(np.full(len(self.vsl_controller.controlzones), 10))
                print ('Speed limit actions applied!')
