...
class VSLControllerGenerator(SumoNet):
    ...
    def get_controller_locations(self):
        ...
        for _junc in self._onramps:
            for edge in self.nodes_info[_junc]['out']:
                ...
                self._search_next_downstream(_startpoint, 0, args.DIST, Zones, Buffers)
    ...
    def _search_next_downstream(self, checkpoint, count, dist, Zones, Buffers):
        _count = count + 1
        if _count <= args.DOWN_VSL_NUM:
            ...
            for path_ in PathsToNext:
                ...
                if self.edges_info[nextcheckpoint[0]]["no.lanes"] < 3:
                    continue
                Zones.append(path_)
                self._search_next_downstream(nextcheckpoint, _count, dist, Zones, Buffers)
...
if __name__ == '__main__':
    ...
    ControlZones, BufferZones = vsl.get_controller_locations()
